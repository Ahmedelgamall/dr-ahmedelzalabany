    ...
    <div class="layout-container">
      ...
      <aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
        ...
        <ul class="menu-inner py-1">
          ...
          <!-- Layouts -->
 @if (auth()->user()->can('list services') ||
         auth()->user()->can('list branches') ||
         auth()->user()->can('list steps') ||
         auth()->user()->can('edit settings') ||
         auth()->user()->can('list branches') ||
         auth()->user()->can('list faqs') ||
...
<li class="menu-item @if (
    ...
    <li class="menu-item">
                                <a href="{{ route('services.index') }}" class="menu-link">
                                  <div data-i18n="Without navbar">{{ getTranslatedWords('services') }}</div>
                                </a>
        </li>
@endcan
    {{--  @can('list branches')
...
@if (auth()->user()->can('list blogs'))
<li class="menu-item @if (\Route::is('blogs.*')) open active @endif">
            <a href="javascript:void(0);" class="menu-link menu-toggle">
              <i class="menu-icon tf-icons bx bx-news"></i>
              <div data-i18n="Layouts">{{ getTranslatedWords('blogs and medical advices') }}</div>
            </a>
...
@endif
            ...
            @if (auth()->user()->can('list appointments') || auth()->user()->can('list contacts'))
<li class="menu-item @if (\Route::is('appointments.*') || \Route::is('contacts.*')) open active @endif">
            <a href="javascript:void(0);" class="menu-link menu-toggle">
              <i class="menu-icon tf-icons bx bx-first-aid"></i>
              <div data-i18n="Layouts">{{ getTranslatedWords('customers services') }}</div>
            </a>
    ...
    <li class="menu-item">
                            <a href="{{ route('appointments.index') }}" class="menu-link">
                              <div data-i18n="Without navbar">{{ getTranslatedWords('appointments') }}</div>
                            </a>
                          </li>
@endcan
              @can('list contacts')
