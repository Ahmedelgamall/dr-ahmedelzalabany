...
class DashboardController extends Controller
{
    ...
    public function change_categories_count(Request $request)
    {
        ...
        if($request->data=='day'){
            $data['total']=$human_readable->format(Order::whereDate('created_at', Carbon::today())->count());
            ...
            $data['count'][]=$human_readable->format(OrderProduct::whereHas('product', function ($q) use ($category) {
                    $q->where('category_id', $category->id);
                                                    })->whereHas('order',function($x){
                                                        $x->whereDate('created_at', Carbon::today());
                                                    })->count()
        ...
        if($request->data=='week'){
            $data['total']=$human_readable->format(Order::whereBetween('created_at', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()])->count());
            foreach(Category::take(6)->get() as $key=> $category){
                $data['fetch_categories'][]=OrderProduct::whereHas('product', function ($q) use ($category) {
                    $q->where('category_id', $category->id);
                                                    })->whereHas('order',function($x){
        ...
        if($request->data=='month'){
            $data['total']=$human_readable->format(Order::whereYear('created_at', Carbon::now()->year)
            ->whereMonth('created_at', Carbon::now()->month)->count());
            foreach(Category::take(6)->get() as $key=> $category){
                $data['fetch_categories'][]=OrderProduct::whereHas('product', function ($q) use ($category) {
                    $q->where('category_id', $category->id);
        ...
        if($request->data=='year'){
            $data['total']=$human_readable->format(Order::whereYear('created_at', Carbon::now()->year)->count());
            foreach(Category::take(6)->get() as $key=> $category){
                $data['fetch_categories'][]=OrderProduct::whereHas('product', function ($q) use ($category) {
                    $q->where('category_id', $category->id);
                                                    })->whereHas('order',function($x){
